...
public:
    ...
    static void ShowFPS(LPDIRECT3DDEVICE8 device)
    {
        ...
        float fps = 0.0f;
        if (m_times.size() >= 2)
            fps = static_cast<float>((static_cast<float>(m_times.size() - 1) * static_cast<float>(frequency.QuadPart)) / static_cast<float>(m_times.back() - m_times.front()));
...
bool WINAPI DllMain(HMODULE hModule, DWORD dwReason, LPVOID lpReserved)
{
    ...
    switch (dwReason)
    {
        case DLL_PROCESS_ATTACH:
        {
            ...
            char fpsLimitBuffer[16] = {};
            GetPrivateProfileStringA("MAIN", "FPSLimit", "0", fpsLimitBuffer, sizeof(fpsLimitBuffer), path);
            fFPSLimit = static_cast<float>(strtod(fpsLimitBuffer, nullptr));
